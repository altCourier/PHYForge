...
if __name__ == "__main__":
    ...
    def make_waveforms(num_time_samples=64, bandwidth=1e6):
    ...
    sys_awgn = PHYSys(cfg_awgn)

    bits, llr = sys_awgn.generate(batch_size=4, num_symbols=64, ebno_db=5.0)

    print("bits:", type(bits), bits.shape, bits.dtype)
    print("llr :", type(llr), llr.shape, llr.dtype)

    print("\n=== TDL + time waveform (exercises reshape) ===")

    num_symbols = 64  # MUST match waveforms.time.num_time_samples -- see point 3

    cfg_tdl = Config(
        common=CommonConfig(active_channel="tdl", active_waveform="time"),
        source=BinarySourceConfig(),
        modulation=make_modulation(),
        channels=make_channels(),
        waveforms=make_waveforms(num_time_samples=num_symbols),
        sweep=SweepConfig(
            ebno_db=EbNoRangeConfig(start=0.0, stop=0.0, step=2.0),
            batch_size=4,
    ...
    sys_tdl = PHYSys(cfg_tdl)

    bits, llr = sys_tdl.generate(batch_size=4, num_symbols=num_symbols, ebno_db=5.0)

    print("bits:", type(bits), bits.shape, bits.dtype)
    print("llr :", type(llr), llr.shape, llr.dtype)

    print("\n=== UMi system-level + time waveform ===")
    ...
    sys_umi = PHYSys(cfg_umi)
    bits, llr = sys_umi.generate(batch_size=4, num_symbols=num_symbols, ebno_db=5.0)

    print("bits:", type(bits), bits.shape, bits.dtype)
    print("llr :", type(llr), llr.shape, llr.dtype)

    b1, l1 = sys_umi.generate(batch_size=4, num_symbols=num_symbols, ebno_db=5.0)
    b2, l2 = sys_umi.generate(batch_size=4, num_symbols=num_symbols, ebno_db=5.0)
    print("llr differs across calls:", not torch.equal(l1, l2))